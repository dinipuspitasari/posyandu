...
@section('title', 'Posyandu Ganggang | Jadwal Posyandu')
...
@section('content')
    ...
    {{-- Judul --}}
    <h2 class="text-xl font-semibold mb-6">Jadwal Posyandu</h2>
    ...
    <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-2 sm:gap-4 mb-4">
        ...
        <form method="GET" action="{{ route('jadwal.index') }}" class="flex items-center">
            ...
            {{-- searching --}}
            <form method="GET" action="{{ route('jadwal.index') }}" class="w-full flex items-center" id="searchForm">
    ...
    <a href="{{ route('jadwal.create') }}"
        ...
        <span class="text-sm">Tambah Jadwal</span>
    </a>
            ...
            <tbody>
                @forelse ($jadwal as $index => $j)
                    <tr class="border-b">
                        ...
                        <td class="px-4 py-2 border">{{ $j->nama_kegiatan }}</td>
                        <td class="px-4 py-2 border">{{ $j->tanggal }}</td>
                        <td class="px-4 py-2 border">{{ $j->waktu }}</td>
                        <td class="px-4 py-2 border">{{ $j->lokasi }}</td>
                        <td class="px-4 py-2 border">{{ $j->petugas->nama ?? '-' }}</td>
                        <td class="px-4 py-2 border">
                            <div class="flex items-center justify-center space-x-1">

                                {{-- Tombol edit --}}
                                <a href={{ route('jadwal.edit', $j->id_jadwal_posyandu) }} type="button"
                                ...
                                <form action="{{ route('jadwal.destroy', $j->id_jadwal_posyandu) }}" method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="ms-2 text-gray-900 bg-red-300 hover:bg-red-300 focus:ring-4 focus:ring-red-400 font-medium rounded-lg text-sm px-1 py-1">
                ...
                @if ($jadwal->isEmpty())
                    <tr>
                        <td colspan="4" class="text-center py-4 text-gray-500">Belum ada data jadwal posyandu.</td>
                    </tr>
                @endif
            </tbody>
        </table>
    ...
    <div class="flex items-center justify-between mt-4 mx-5">
        <p class="text-gray-900 text-sm">
            Showing {{ $jadwal->firstItem() ?? 0 }} to {{ $jadwal->lastItem() ?? 0 }} of
            {{ $jadwal->total() }} entries
        </p>
        ...
        <div class="flex space-x-1">
            ...
            {{-- Halaman aktif --}}
            <span class="px-3 py-1 bg-blue-600 text-white rounded text-sm">{{ $jadwal->currentPage() }}</span>
