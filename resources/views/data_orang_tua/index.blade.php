...
@section('title', 'Posyandu Ganggang | Data Orang Tua')
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-700">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="px-4 py-2 border">NIK Ibu</th>
                        <th class="px-4 py-2 border">Nama Ibu</th>
                        <th class="px-4 py-2 border">No Telpon</th>
                ...
                <tbody>
                    @forelse($orangTua as $index => $item)
                        <tr class="border-b">
                            ...
                            <td class="px-4 py-2 border">{{ $item->nik_ibu }}</td>
                            <td class="px-4 py-2 border">{{ $item->nama_ibu }}</td>
                            <td class="px-4 py-2 border">{{ $item->no_telpon }}</td>
                            <td class="px-4 py-2 border">{{ $item->alamat }}</td>
                            <td class="px-4 py-2 border">
                                <div class="flex items-center space-x-1">

                                    {{-- Tombol Edit --}}
                                    <a href={{ route('data_orang_tua.edit', $item->id_data_orang_tua) }} type="button"
                                        class="ms-2 text-gray-900 bg-blue-300 hover:bg-blue-300 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-1 py-1">
                                    ...
                                    {{-- Tombol Hapus --}}
                                    <form action="{{ route('data_orang_tua.destroy', $item->id_data_orang_tua) }}" method="POST"
                                        onsubmit="return confirm('Anda Yakin ingin Menghapus {{ $item->nama_ibu }} ?')"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-4 text-gray-500">Belum ada data orang tua.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
